            ...
            for (var key in xyrObj) {
                if (k == key) {
                    ...
                    for (var i = 0; i < xydxDatas[k].length; i++) {
                        if (key == 'TB_ST_RY_WFXYRY') {//违法嫌疑人的显示组合信息
                            var xyrzhxx = filedToParagraphXyr(xydxDatas[k][i]);
                        ...
                        } else if (key == 'TB_ST_DW_WFXYDW') {//违法嫌疑单位
                            xyrStr += '<li><label><input xxzjbh="' + xydxDatas[k][i][xyrObj[key].xxzjbh] + '" type="checkbox"/>' +
                                '<span xyrtype="' + xyrObj[key].id + '">' + xydxDatas[k][i][xyrObj[key].param] + '</span></label></li>';
                        } else if (key == 'TB_ST_RY_AJXGRY') {//案件相关人员
                            xyrStr += '<li><label><input xxzjbh="' + xydxDatas[k][i][xyrObj[key].xxzjbh] + '" type="checkbox"/>' +
                                '<span xyrtype="' + xyrObj[key].id + '">' + xydxDatas[k][i][xyrObj[key].param] + '</span></label></li>';
                        }
                    }
...
/**
 * 违法嫌疑人组合信息  字段组合成文字段信息
 * @param xyrinfo
 * @returns {string}
 */
function filedToParagraphXyr(xyrinfo) {
...
function xyrCheckedXxfy($this) {
    ...
    if (parentDiv.find('input:checked').length > 0) {//选中
        ...
        //违法嫌疑人
        var zhxxObj = null;
        if (parentUl.attr("ids") == "wfxyr" && (DATA.CQBG.cqbgData.xyrpz != undefined || DATA.CQBG.cqbgData.xyrpz != '')) {//勾选嫌疑人
            for (var key in DATA.DX.xydxData.TB_ST_RY_WFXYRY) {
                if (DATA.DX.xydxData.TB_ST_RY_WFXYRY[key].xxzjbh == $this.attr("xxzjbh")) {
                    zhxxObj = DATA.DX.xydxData.TB_ST_RY_WFXYRY[key];
                }
            }
            var zhxx = {};
            var xyrpzArr = DATA.CQBG.cqbgData.xyrpz.split(",");
        ...
        //违法嫌疑单位
        else if (parentUl.attr("ids") == "wfxyrdw" && (DATA.CQBG.cqbgData.xydwpz != undefined || DATA.CQBG.cqbgData.xydwpz != '')) {//勾选嫌疑单位
            for (var key in DATA.DX.xydxData.TB_ST_DW_WFXYDW) {
                if (DATA.DX.xydxData.TB_ST_DW_WFXYDW[key].xxzjbh == $this.attr("xxzjbh")) {
                    zhxxObj = DATA.DX.xydxData.TB_ST_DW_WFXYDW[key];
                }
            }

            var zhxx = {};
            var xyrpzArr = DATA.CQBG.cqbgData.xydwpz.split(",");
...
/**
 * 呈请报告|法律文书 多个犯罪嫌疑人列表同一时间只能操作一个
 * @param tmpId 父级容器id
 */
function xyrListControl(tmpId) {
    $('#' + tmpId + ' .xyrList input:checkbox').off('click').on('click', function () {
        if (DATA.ajax.count != 0) {
            return;
        }
        var $this = $(this);
            ...
            if (flwsRow[0].CLDXLB) {
                ...
                for (var i = 0; i < flwsRow.length; i++) {
                    ...
                    xyrStr += '<li><label ' + title + ' class="easyui-tooltip"><input xxzjbh="' + flwsRow[i].CLDX_XXZJBH + '" flwszj="' + flwsRow[i].ZJ + '" ' + disabled + ' type="checkbox"/>' +
                        '<span xyrtype="' + xyrObjTemp.id + '">' + flwsRow[i][(xyrObjTemp.param).toUpperCase()] + '</span></label></li>';
                }
                ...
                for (var k in xydxDatas) {
                    for (var key in xyrObj) {
                        if (k == key) {
                            var xyrStr = '';
                            for (var i = 0; i < xydxDatas[k].length; i++) {
                                if (key == 'TB_ST_RY_WFXYRY') {//嫌疑人的显示组合信息
    ...
    if (xydxDatas) {
        ...
        if (DATA.FLWS[bm].flwsRow.length > 0) {
            ...
            for (var i = 0; i < tmpArray.length; i++) {
                $('#flws_xyr_area_' + bm).find("input[xxzjbh='" + tmpArray[i] + "']").prop('checked', true);
            }
        }
...
function flwsWclXyrCheck(bm, $this, event) {
    ...
    if (parentDiv.find('input:checked').length > 0) {//选中
        ...
        if (DATA.FLWS[bm].flwsData.dx) {//1
            ...
            if (parentUl.attr("ids") == "wfxyr" && (flwsData.xyrpz != undefined || flwsData.xyrpz != '')) {//勾选嫌疑人
                for (var key in DATA.DX.xydxData.TB_ST_RY_WFXYRY) {
                    if (DATA.DX.xydxData.TB_ST_RY_WFXYRY[key].xxzjbh == $this.attr("xxzjbh")) {
                        zhxxObj = DATA.DX.xydxData.TB_ST_RY_WFXYRY[key];
                    }
            ...
            } else if (parentUl.attr("ids") == "wfxyrdw" && (flwsData.xydwpz != undefined || flwsData.xydwpz != '')) {
                for (var key in DATA.DX.xydxData.TB_ST_DW_WFXYDW) {
                    if (DATA.DX.xydxData.TB_ST_DW_WFXYDW[key].xxzjbh == $this.attr("xxzjbh")) {
                        zhxxObj = DATA.DX.xydxData.TB_ST_DW_WFXYDW[key];
                    }
