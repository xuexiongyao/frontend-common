 ...
 * easyui初始化组件
 */
function easyuiReset(ipts,isAdd){
    var urlAttr={}
    ...
    for(var i=0;i<ipts.length;i++){
        ...
        if(annotation){
            ...
            if($(ipts[i]).hasClass('easyuicombobox')){//combobox字典
                ...
                var combotreeObj = {
                    showText: true,onlyLeaf:true,dataFilter:'',method:'get',editable:false,multiple:false,lines:true,required:true,
                    onChange:  function (newValue,oldValue) {
                        if(newValue){
                            var $this = $(this);
                            var className = $this.attr('textboxname');//组件class name值
                                ...
                                onChange: function (newValue,oldValue) {
                                    ...
                                    if (!isNaN(iptVal)) {
                                        ...
                                        $.messager.alert({
                                            ...
                                            msg: '只能输入数字',
                                            icon: 'warning',
                                            fn: function () {$this.textbox({value: ''})}
                                        })
                                    }
                                }
                            });
