            ...
            success: function (json) {
                if (json) {
                    ...
                    for (var key in DATA.DX.xydxData) {
                        ...
                        if (DATA.DX.xydxData["TB_ST_RY_WFXYRY"] != undefined) {
                            var fzxyr_rsqzcsdm;
                            for (var i = 0; i < DATA.DX.xydxData["TB_ST_RY_WFXYRY"].length; i++) {
                                var fzxyr = DATA.DX.xydxData["TB_ST_RY_WFXYRY"][i];
                                if(fzxyr.fzxyr_rsqzcsdm == null || fzxyr.fzxyr_rsqzcsdm == 'null'){
                                    fzxyr_rsqzcsdm = '';
                                }else{
                                    fzxyr_rsqzcsdm = fzxyr.fzxyr_rsqzcsdm;
                                }
                                var rule = DATA.RULE[fzxyr_rsqzcsdm];
                                var disabled = "";
                                var title = "";
                                if (rule != undefined) {
                                    if (rule.iscontain) {
