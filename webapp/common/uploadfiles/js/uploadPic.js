...
(function( $ ) {
	...
	function bottomBtn(){
		$('#cancel_all').off('click').on('click',function(){
	...
	function createBox( $fileInput, file, webUploader ) {
		...
		if ( $parentFileBox.find('.diyButton').length <= 0 ) {
			...
			$('#upload_pic').one('click',uploadStart);
		...
		//创建按钮 easyui--Div
		if ( $('.btn-area').length <= 0 ) {
			var div = '<div class="btn-area"> \
						<a class="easyui-linkbutton c6" id="upload_file">开始上传</a>\
						<a class="easyui-linkbutton c6" id="cancel_all">全部取消</a>\
					</div>';
			$parentFileBox.after( div );
			var $startButton = $('#upload_file');
			var $cancelButton = $('#cancel_all');;
			$startButton.linkbutton();
			$cancelButton.linkbutton();
			//开始上传,暂停上传,重新上传事件;
			var uploadStart = function (){
				webUploader.upload();
				$startButton.linkbutton({text : '暂停上传'}).one('click',function(){
					webUploader.stop();
					$(this).linkbutton({text : '继续上传'}).one('click',function(){
						uploadStart();
					});
				});
			};
		...
		if($('#add_file').length == 0){
			li += '<li style="cursor:pointer" title="点击添加" id="add_file"></li>';
			$parentFileBox.children('.fileBoxUl').append( li );
		}else{
			$('#add_file').before(li);
		}

		$('#add_file').off('click').on('click',function(){
			$('.webuploader-element-invisible').click();
		});

		//console.log($('#add_file'));

		//$parentFileBox.children('#add_file').before( li );
